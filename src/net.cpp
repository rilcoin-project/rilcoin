...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Rilcoin " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"rilcoincrypto.org", "217.182.224.94"},
    {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Rilcoin is probably already running."), addrBind.ToString().c_str());
        else
