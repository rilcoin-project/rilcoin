...
*Setup instructions for a gitian build of Rilcoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Rilcoin
...
- [Building Rilcoin](#building-rilcoin)
...
Clone the git repositories for rilcoin and gitian and then checkout the rilcoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the rilcoin repository
...
To build Rilcoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the rilcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/rilcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/rilcoin-project/rilcoin
...
For example:
```bash
URL=https://github.com/user/rilcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit rilcoin=${COMMIT} --url rilcoin=${URL} ../rilcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit rilcoin=${COMMIT} --url rilcoin=${URL} ../rilcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit rilcoin=${COMMIT} --url rilcoin=${URL} ../rilcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/rilcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/rilcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/rilcoin-build.assert
```
...
[rilcoin/gitian.sigs](https://github.com/rilcoin-project/gitian.sigs.ril/) repository, or if that's not possible create a pull
request. 
